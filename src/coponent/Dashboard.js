...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const url = `http://localhost:8081/api/incidents/filtered?${category}=${selectedYear}`;
      try {
        const response = await fetch(url);
        if (response.ok) {
          const data = await response.json();

          // Prepare arrays to hold data for Intensity, Likelihood, and Relevance
          ...
          setUserData({
            labels: data.map((item) => item.end_year), // Assuming "end_year" is common for all data points
            datasets: [
              {
                label: "Intensity",
                data: intensityData,
